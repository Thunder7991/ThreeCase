...
gui.add(colorParams,"r").min(-1).max(1).step(0.01).onChange((r) => {
  shaderPass.uniforms.uColor.value.r = r
})
gui.add(colorParams,"g").min(-1).max(1).step(0.01).onChange((g) => {
  shaderPass.uniforms.uColor.value.g = g
})
gui.add(colorParams,"b").min(-1).max(1).step(0.01).onChange((b) => {
  shaderPass.uniforms.uColor.value.b = b
})
